...
class goarch_class {
	public function __construct() {
		...
		$this->goarch_includes();
		/**
		 * Hooks
		 */
		add_action( 'after_setup_theme', array( $this, 'goarch_crucial_setup' ) );

		// widgets
		add_action( 'widgets_init', array( $this, 'goarch_arphabet_widgets_init' ) );
		add_action( 'current_screen', array( $this, 'goarch_my_theme_add_editor_styles' ) );
		//filters
		add_filter( 'body_class', array( $this, 'goarch_add_body_class' ) );
		//theme support
		$this->goarch_theme_support_setting();
	 ...
	 */
	function goarch_theme_support_setting() {
		...
		add_image_size( "goarch_post", 849, 450, true );
		add_image_size( "goarch_760_514", 760, 514, true );
		add_image_size( "goarch-image-480x880-croped", 480, 880, true );


		register_nav_menus(
			array(
				'goarch_topmenu' => esc_html__( 'Header menu', 'goarch' ),
	...
	function goarch_includes() {
		...
		require get_template_directory() . '/assets/custom-style.php';
	 ...
	 *                           Hooks Action
	 ************************************************************/
	function goarch_crucial_setup() {
		load_theme_textdomain( 'goarch', get_template_directory() . '/languages' );
	 ...
	 *
	 */
	function goarch_arphabet_widgets_init() {
		register_sidebar( array(
			'name' => esc_html__( 'sidebar', 'goarch' ),
			'id' => 'goarch_sidebar',
			'before_widget' => '<div id="%1$s" class="widget   %2$s">',
			'after_widget' => '</div>',
			'description' => esc_html__( 'blog sidebar', 'goarch' ),
	 ...
	 * @return array
	 */
	function goarch_add_body_class( $classes ) {
	 ...
	 * @return string
	 */
	function goarch_container_class() {
	...
	function goarch_my_theme_add_editor_styles() {
	 ...
	 */
	function goarch_google_fonts_url( $fonts_param ) {
		$font_url = '';
		/*
		Translators: If there are characters in your language that are not supported
		by chosen font(s), translate this to 'off'. Do not translate into your own language.
		 */
		if ( 'off' !== esc_html_x( 'on', 'Google font: on or off', 'goarch' ) ) {
	 ...
	 */
	function goarch_get_post_thumbnail( $theid = null, $widht = 848, $height = 450, $big_src = false, $default = true, $return = false ) {
		...
		if ( $big_src ) {
			...
			if ( function_exists( 'goarch_resize_image' ) ) {

				$thumb_arr = goarch_resize_image( get_post_thumbnail_id( $theid ), $widht, $height, true );
				$thumb = $thumb_arr['url'];
	 ...
	 */
	function goarch_pagenavi( $max_page = false, $before = '', $after = '', $echo = true, $args = array(), $wp_query = null ) {
		if ( !$wp_query ) {
			global $wp_query;
		}
		...
		$default_args = array(
			...
			'back_text' => '<i class="fa fa-angle-double-left"></i> ' . esc_html__( 'Previous', 'goarch' ),
			// text "go to the previous page." Put 0 if this reference is not needed
			'next_text' => esc_html__( 'Next', 'goarch' ) . ' <i class="fa fa-angle-double-right"></i>',
		...
		$default_args = apply_filters( 'goarch_pagenavi_args', $default_args ); //to be able to establish their default values
...
$GLOBALS['goarch_class'] = new goarch_class();

/**
 * @return mixed
 */
function goarch_get_global_class() {
	global $goarch_class;
	return $goarch_class;
}

add_filter( 'get_the_excerpt', 'goarch_exc', 90 );
/**
 * carves out a brief description of shortcodes
 * @param $param
 * @return mixed
 */
function goarch_exc( $param ) {
 ...
 * @return mixed
 */
function goarch_newBasename( $path = false, $is_page = false ) {
 ...
 * coments pagination
 */
function goarch_wp_comments_corenavi() {
	$pages = '';
...
if ( !function_exists( 'goarch_get_youtube_id' ) ) {

	/**
	 * @param $value
	 * @return mixed|null
	 */
	function goarch_get_youtube_id( $value ) {
		$id = null;
 ...
 * @return mixed|string
 */
function goarch_video_patern( $carry, $item ) {
	if ( strpos( $item, '#' ) === 0 ) {
		// Assuming '#...#i' regexps.
...
if ( !function_exists( 'goarch_theme_oembed_videos' ) ) :

	function goarch_theme_oembed_videos() {
		...
		if ( $post && $post->post_content ) {
			...
			$shortcode_tags = apply_filters( 'goarch_vide_embed_tags', array(
			...
			$pattern = '#(' . array_reduce( $pattern_array, "goarch_video_patern" ) . ')#is';
			...
			foreach ( $lines as $line ) {
				...
				if ( preg_match( $pattern, $line, $matches ) ) {
					if ( strpos( $matches[0], '[' ) === 0 ) {
						...
						$ret = '<' . 'iframe' . ' src="https://www.youtube.com/embed/' . sanitize_text_field( goarch_get_youtube_id( $matches[0] ) ) . '?feature=oembed" class="youtube_video"  allowfullscreen></iframe>';
...
if ( !function_exists( 'goarch_post_gallery_slide' ) ) :
	 ...
	 * @param int $height
	 */
	function goarch_post_gallery_slide( $cut = false, $width = 848, $height = 566 ) {

		global $goarch_class, $post;
        ...
        <div class="gallery-carousel carousel">
			...
			if ( has_post_thumbnail() ) {
				...
				$feature_src = wp_get_attachment_image_src( $post_thumbnail_id, 'goarch_post' );
...
if ( !function_exists( 'goarch_limit_excerpt' ) ) :
	function goarch_limit_excerpt( $limit, $content = null ) {
...
if ( !function_exists( 'goarch_words_limit' ) ) :

	function goarch_words_limit() {
		$limit = get_theme_mod( 'goarch_blog_list_limit_word' );
 ...
 * @return string
 */
function goarch_resize_image( $thumb_id, $width, $height, $crop ) {
 ...
 * @return mixed
 */
function goarch_get_current_price_product( $product_id ) {
 ...
 * @return string
 */
function goarch_social_menu_item( $items, $args ) {
	...
	if ( ( $args->theme_location == 'goarch_topmenu' || $args->theme_location == 'goarch_topmenu_blog' ) ) {


		if ( !is_front_page() && get_option( 'goarch_one_page_menu_' . $post_ID ) == false ) {
			// $menu_left = get_option('goarch_one_page_menu');
			//  $menu_left = str_replace('#', get_home_url('/') . '/#', $menu_left);

		} else {


			$menu_left = get_option( 'goarch_one_page_menu_' . $post_ID );
	...
	if ( get_option( 'goarch_one_page_menu_right' ) == true && ( $args->theme_location == 'goarch_topmenu' || $args->theme_location == 'goarch_topmenu_blog' ) ) {

		if ( !is_front_page() && get_option( 'goarch_one_page_menu_right_' . $post_ID ) == false ) {
			$menu_right = get_option( 'goarch_one_page_menu_right' );;
			...
			$menu_right = get_option( 'goarch_one_page_menu_right_' . $post_ID );
...
add_filter( 'wp_nav_menu_items', 'goarch_social_menu_item', 10, 2 );


/*
 *  it display lis of post cat
 */
function goarch_get_list_cats() {
	global $post;
 ...
 * @return string
 */
function goarch_get_tememe_color() {
...
add_filter( 'nav_menu_css_class', 'goarch_special_nav_class', 10, 2 );

function goarch_special_nav_class( $classes, $item ) {
...
if ( !function_exists( 'goarch_post_thumbnail' ) ) :
	/**
	 ...
	 * @since Rent It 1.0
	 */
	function goarch_post_thumbnail() {
		global $goarch_class;

		///$theid = null, $widht = 848, $height = 565, $big_src = false, $default = true, $return = false
		if ( $goarch_class->goarch_get_post_thumbnail( get_the_ID(), 0, 0, false, true, true ) == false || post_password_required() || is_attachment() || !has_post_thumbnail() ) {
			return;
		}
		if ( strlen( $goarch_class->goarch_get_post_thumbnail( get_the_ID(), 0, 0, 0, 0, 1 ) ) < 5 ) {
			return false;
		}
		?>

        <img class="img-responsive"
             src=" <?php echo esc_url( $goarch_class->goarch_get_post_thumbnail( get_the_ID() ) ); ?>"
             alt="<?php the_title(); ?>  ">
